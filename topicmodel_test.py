...
class TopicModel(object):
    ...
    def process_all_documents(self, lib):
        ...
        for loop in collection.find({}):
            ...
            if(i == 5):
                self.test_raw = loop["code"]
            # if(i==10):   # just for test
            #     break
            i = i+1
    ...
    def build_topicModel(self, corpus, dictionary):
        ...
        lsi = models.LdaModel(corpus = corpus, id2word=dictionary
                     , num_topics = 50
        ...
        self.print_chart(lsi)
        print "==== ldamodel constructed ===="
        return lsi
        
    def print_chart(self, lda):
        ...
        for item in lda.show_topics():
            if j ==5:
                break
            m = 0
            for word in lda.show_topic(j):
                # print word[0], word[1]
                topic_tfidf.append(word[1])
            
            ax[j].stem(topic_tfidf, linefmt='b-',    
                    markerfmt='bo', basefmt='w-')    
            ax[j].set_xlim(-1,20)    
            ax[j].set_ylim(0, 0.01)    
            ax[j].set_ylabel("Prob")    
            ax[j].set_title("topic {}".format(j))    
            topic_tfidf = []
            j = j+1
    ...
    def testTopicLen(self, lsi, corpus):
        ...
        print "avg num of topic:",np.mean(lens), "num of article has one topic:",np.mean(lens==1)
        for topic in topics:
            print "topics in one article:===>","\n",topic
        # topics = [lsi[c] for c in corpus]
        # print topics
    ...
    def train(self, env):    # 训练全部主题模型
        # self.get_libs()
        processor = Processor()
        print "processing new model...", LIB_NAME
        if env == 1:
            self.process_all_documents(LIB_NAME)
        else:
            self.testVector()
        texts_after = processor.rm_tokens(self.posts)   # 得到去除停用词的文档二维数组
        self.texts_after = texts_after
    ...
    def build_index(self):
        ...
        index = similarities.MatrixSimilarity(self.lsi[self.corpus])   # 将语料库映射到主题模型空间中
